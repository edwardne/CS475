...
int main(int argc, char *argv[]){
    ...
    for (int i = 0; i < NUMNODES*NUMNODES; i++) {
        ...
        if (iu == NUMNODES-1 || iu == 0){
            if (iv == NUMNODES-1 || iv == 0){
                weight = 0.25;
            } else {
                weight = 0.5;
            }
        ...
        float dArea = fullTileArea * weight;

        float dVolume = dArea * Height(iu, iv);
    ...
    printf("totalVolume = %8.4lf\n",totalVolume);
    printf("kiloHeights/sec = %8.2lf\n", NUMNODES*NUMNODES/1000/(time1-time0));
